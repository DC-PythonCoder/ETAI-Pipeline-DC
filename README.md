...
## Week 3 Work I Did
I added a data_diagnostic file to perform the whole EDA step of the pipeline and discover conclusions on how to clean the dataset, then I added this conclusions to the "config.yaml" file in the "diagnostic" part. Afterwards I added a function to clean the dataset to the "preprocessing.py" file along with the canonicalize categories function and the necessary imports. I then used this "diagnostic" information to clean the dataset by calling the clean dataset function in the "main.py" file using the information on the "diagnostic" part of the "config.yaml" file. Finally I run "python main.py" in the terminal, got the results, and I found that both the train and test accuracy became very slightly worst than both logistics regression and DT models, and the precision, which is a better measure for the purpose of this project since I want to minimize false positives, became also very slightly worst than in the logistics regression model, but has the same values as the DT.
